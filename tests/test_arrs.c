...
int test_arrs(){
	...
	// can create a new array
	{
		...
		void* d = get_new_arr(&arrs, 10, ARR_DIM2_UNUSED);
		ASSERT(arrs.arr_data == d, "[get_new_arr] First array has pointer to start of data");
	...
	// array indexing
	{
		...
		void* d = get_new_arr(&arrs, 3, ARR_DIM2_UNUSED);
		ASSERT(arrs.arr_data == d, "[get_new_arr] First array has pointer to start of data");
		ASSERT(arrs.arr_data_next == 5, "[get_new_arr] Next points to correct size");
