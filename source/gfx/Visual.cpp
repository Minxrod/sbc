...
void Visual::colinit_(const Args& a){
	...
	if (a.size() > 1){
		...
		load_default(r.col.at("COL0U"), "resources/graphics/COL0.PTC");
		load_default(r.col.at("COL1U"), "resources/graphics/COL1.PTC");
		load_default(r.col.at("COL2U"), "resources/graphics/COL2.PTC");
		load_default(r.col.at("COL0L"), "resources/graphics/COL0.PTC");
		load_default(r.col.at("COL1L"), "resources/graphics/COL1.PTC");
		load_default(r.col.at("COL2L"), "resources/graphics/COL2.PTC");
	}
	regen_col();
}
...
/// PTC command to reset (most of) the entire graphics state.
...
/// @todo Dialog is not implemented, so the silent flag is currently ignored.
/// 
/// Format: 
...
void Visual::draw(sf::RenderWindow& w){
	...
	for (int sc = 0; sc < 2; ++sc){
		...
		//bg
		for (int l = 1; l >= 0; --l){
			auto state = set_state(Tex::BGU_U + chr_l, COL_BG + col_l);
